...
from billing.models import CreditTransaction, Payment
...
def client_detail(request, user_id):
    ...
    stats = scrub_jobs.aggregate(
        total_numbers=Sum('total'),
        total_clean=Sum('clean'),
        total_dnc=Sum('dnc'),
    )
    total_spent = payments.filter(status='completed').aggregate(s=Sum('amount'))['s'] or 0
